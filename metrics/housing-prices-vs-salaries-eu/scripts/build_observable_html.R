...
<body>
  ...
  <script type="module">
    ...
    function render() {
      ...
      const height = rows.length * (compact ? 29 : 33) + 56;
      const xMin = Math.min(-70, Math.floor(Math.min(...rows.map(d => d.change)) / 10) * 10 - 10);
      const xMax = Math.max(55, Math.ceil(Math.max(...rows.map(d => d.change)) / 10) * 10);

      const plot = Plot.plot({
        width,
        height,
        marginLeft: compact ? 118 : 150,
        marginRight: compact ? 44 : 72,
        marginTop: 10,
        marginBottom: 32,
        style: {
          background: "transparent",
