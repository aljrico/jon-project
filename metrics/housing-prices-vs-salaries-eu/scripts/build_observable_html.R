...
<body>
  <main>
    ...
    <p class="subtitle">Incremento del precio de la vivienda ajustado al crecimiento de los salarios, 2015-2024. Valores positivos indican que la vivienda se encareció más rápido que los salarios. La línea punteada marca la media de la UE.</p>
  ...
  <script type="module">
    ...
    function render() {
      ...
      const marginTop = compact ? 26 : 30;
      const height = rows.length * (compact ? 29 : 33) + 76;
      const changes = euRow ? rows.map(d => d.change).concat(euRow.change) : rows.map(d => d.change);
      ...
      const plot = Plot.plot({
        ...
        marks: [
          Plot.ruleX([0], {stroke: "#9aa7b3", strokeWidth: 1}),
          euRow ? Plot.ruleX([euRow.change], {stroke: "#7d8a96", strokeWidth: 1.25, strokeDasharray: "4,4"}) : null,
          Plot.barX(rows, {
            y: "country",
            x: "change",
      ...
      if (euRow) {
        addEuAverageLabel(plot, euRow, xMin, xMax, width, marginLeft, marginRight, compact);
      }
      addCountryLabels(plot, rows, compact);
      addValueLabels(plot, rows, compact);

      installTooltip(plot);
    }

    function addEuAverageLabel(plot, euRow, xMin, xMax, width, marginLeft, marginRight, compact) {
      const svg = plot;
      const ns = "http://www.w3.org/2000/svg";
      const plotWidth = width - marginLeft - marginRight;
      const x = marginLeft + ((euRow.change - xMin) / (xMax - xMin)) * plotWidth;
      const label = document.createElementNS(ns, "text");

      label.textContent = `UE ${fmtPct(euRow.change)}`;
      label.setAttribute("x", String(x + 5));
      label.setAttribute("y", compact ? "15" : "17");
      label.setAttribute("text-anchor", "start");
      label.setAttribute("fill", "#7d8a96");
    ...
    function addCountryLabels(plot, rows, compact) {
      const svg = plot;
      const ns = "http://www.w3.org/2000/svg";
      ...
      bars.forEach((bar, i) => {
        ...
        label.setAttribute("fill", "#26323d");
        label.setAttribute("font-weight", d.geo === "ES" ? "800" : "400");
